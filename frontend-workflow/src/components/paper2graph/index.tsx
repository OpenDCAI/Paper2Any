          ...
          {enableDrawio && drawioXml && (
            <div className="mb-10">
              {drawioXml === emptyDrawioXml ? (
                <div className="portal-card-strong rounded-2xl border border-primary-100 p-4">
                  <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div>
                      <h3 className="text-sm font-semibold text-[var(--text-primary)]">DrawIO 在线编辑 / Editor</h3>
                      <p className="text-xs text-[var(--text-secondary)]">可直接在下方编辑图形，支持复制或下载 .drawio / Edit below and download .drawio</p>
                    </div>
                    <span className="text-[11px] text-[#8b726b]">等待生成 / Pending</span>
                  </div>
                  <div
                    className="portal-preview-canvas mt-4 flex flex-col items-center justify-center rounded-2xl"
                    style={{ height: '560px' }}
                  >
                    <svg className="mb-4 h-16 w-16 text-primary-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                    ...
                    <p className="text-sm text-[var(--text-secondary)]">请先上传论文并生成模型架构图</p>
                    <p className="mt-1 text-xs text-[#8b726b]">Upload a paper and generate the model architecture first</p>
                  </div>
                </div>
              ) : (
                <DrawioInlineEditor
