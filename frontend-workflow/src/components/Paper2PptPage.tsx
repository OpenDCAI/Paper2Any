...
const Paper2PptPage = () => {
  ...
  const [genFigModel, setGenFigModel] = useState('gemini-2.5-flash-preview-05-20');
    ...
    try {
      ...
      const pagecontent = outlineData.map((slide, idx) => {
        ...
        if (result?.afterImage) {
          generatedPath = result.afterImage.replace(
            'http://127.0.0.1:8000/outputs/',
            '/home/ubuntu/szl/DataFlow-Agent/outputs/'
          );
        }
        return {
          title: slide.title,
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass rounded-xl border border-white/10 p-6 space-y-4">
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1">风格提示词（可选）</label>
            <textarea 
              value={globalPrompt} 
              onChange={e => setGlobalPrompt(e.target.value)} 
              placeholder="例如：使用紫色系配色，保持学术风格..." 
              rows={2} 
  ...
  return (
    <div className="w-full h-screen flex flex-col bg-[#050512] overflow-hidden">
      {showBanner && (
        <div className="w-full bg-gradient-to-r from-purple-600 via-pink-600 to-rose-500 relative flex-shrink-0">
          <div className="absolute inset-0 bg-black opacity-20"></div>
          <div className="relative max-w-7xl mx-auto px-4 py-2.5 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Star size={14} className="text-yellow-300 fill-yellow-300" />
              <span className="text-sm text-white">✨ Paper2PPT - 论文智能转 PPT</span>
            </div>
            <div className="flex items-center gap-2">
              <a href="https://github.com/OpenDCAI/DataFlow-Agent" target="_blank" rel="noopener noreferrer" className="px-3 py-1 bg-white/90 text-gray-900 rounded-full text-xs font-medium flex items-center gap-1">
