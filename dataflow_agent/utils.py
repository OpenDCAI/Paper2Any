...
# Optional heavy deps: keep import-time light for minimal deployments (e.g. HF Spaces).
try:  # pragma: no cover
    ...
    PP_ALIGN = RGBColor = MSO_SHAPE = Inches = Pt = None  # type: ignore
...
try:  # pragma: no cover
    from dataflow_agent.toolkits.imtool.mineru_tool import (  # type: ignore
        run_aio_batch_two_step_extract,
        run_aio_two_step_extract,
    )
except Exception:  # pragma: no cover
    run_aio_batch_two_step_extract = None  # type: ignore
    run_aio_two_step_extract = None  # type: ignore
...
# Keep MinerU-only helpers optional.
import asyncio
from pathlib import Path
...
try:  # pragma: no cover
    from mineru_vl_utils import MinerUClient as _MinerUClient  # type: ignore
except Exception:  # pragma: no cover
    _MinerUClient = None  # type: ignore
