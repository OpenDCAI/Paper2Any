...
请遵循以下严格规则：
1. **不改结构**：严禁新增、删除、合并或重排页面。输出 JSON 数组长度必须与输入完全一致，且第 i 个对象必须对应输入的第 i 页（按索引对齐）。
2. **只改内容**：仅允许修改每页内容字段：`title` / `layout_description` / `key_points`。
3. **保留引用**：默认保留 `asset_ref`（以及其它非内容字段），除非用户反馈明确要求修改。
4. **反幻觉**：禁止编造论文中不存在的具体事实、数值、指标、结论或对比结果。若原文未提供支撑信息，只能做结构化补充（例如补充讲述维度/表达更完整），不能捏造细节。
5. **格式严格**：输出必须且只能是标准 JSON 数组。严禁包含 markdown 标记（如 ```json）、前言、后语或任何非 JSON 字符。
6. **最小必要修改**：仅修改反馈涉及的页面与要点；未涉及页面保持原样。
"""
...
**约束条件：**
1. 目标PPT页数： {page_count} 页（输出必须与输入等长，按索引逐页对应）。
2. 不允许新增/删除/重排页面；仅允许修改 `title` / `layout_description` / `key_points`。
3. `asset_ref` 默认保留；除非用户反馈明确要求修改。
4. 返回论文内容一致的语言；
5. 若用户提到“第 N 页”，按 1-based 页码理解：输入数组第 1 个对象为“第 1 页” 或 “第一页”。
6. 如果需要添加内容，则必须严格参考论文内容，绝对不能添加论文中不存在的内容或数据！！
