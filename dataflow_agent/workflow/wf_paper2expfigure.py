...
# from dataflow_agent.states.Paper2FigureState import Paper2FigureState
...
def create_paper2expfigure_graph() -> GenericGraphBuilder:  # noqa: N802
    """
    Workflow factory: dfa run --wf paper2expfigure
    """
    ...
    def _purpose(state: Paper2FigureState):
        return "这里放入字符串 / 数值 / 列表 / 字典等供 prompt 使用"

    # 后置工具就是让agent选择的工具，可以定制多个；
    @builder.post_tool("step2")
    @tool
    def _example_post_tool(date_str: str):
        """
        示例后置工具：根据日期获取天气信息

        Args:
            date_str: 日期字符串，格式为 "MM-DD"。
                例如: "11-29" 表示 11 月 29 日。

        Returns:
            一个描述该日期天气状况的中文字符串。
            示例返回值: "明天天气晴朗，可以放心出行。"
        """
        # 这里只是 demo，真实逻辑中请根据 date_str 查询实际天气
        return "明天天气晴朗，可以放心出行。"
    ...
    # NODES
    # ==============================================================
    async def step1(state: Paper2FigureState) -> Paper2FigureState:
        """
    ...
    async def step2(state: Paper2FigureState) -> Paper2FigureState:
        """
        示例节点 2: 处理agent执行结果
