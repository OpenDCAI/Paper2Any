...
# NOTE: `mineru_vl_utils` is optional for some deployments (e.g. HF Spaces without PDF/MinerU).
# Keep imports lazy so that non-MinerU workflows can still run.
try:  # pragma: no cover
    from mineru_vl_utils import MinerUClient  # type: ignore
except Exception:  # pragma: no cover
    MinerUClient = None  # type: ignore
...
def run_two_step_extract(image_path: str, port: int):
    ...
    if MinerUClient is None:
        raise RuntimeError(
            "`mineru_vl_utils` is not installed. Install it to use MinerUClient-based extraction."
        )
