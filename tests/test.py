...
def create_pipeline_graph() -> GenericGraphBuilder:
    ...
    async def classifier_node(s: DFState) -> DFState:
        ...
        s.temp_data['split_ops'] = get_operators_by_rag(
                ...
                chat_base_url=s.request.chat_api_url,
                chat_model=s.request.model,
        )
    ...
    async def op_exp_node(s: DFState) -> DFState:
        ...
        op_explanations_dict = explain_operator_sources(ops_list, base_url=s.request.chat_api_url, model=s.request.model)
