...
def parse_args():
    ...
    # 缓存目录，用于存储测试数据和提示词
    p.add_argument('--cache-dir',     default=None, help='File path for cache directory')
...
async def main():
    ...
    req = DFRequest(
        ...
        python_file_path=args.cache_dir,
    )
    ...
    state.temp_data["pipeline_file_path"] = args.file_path
...
# python /mnt/DataFlow/lz/proj/agentgroup/ziyi/dataflow/DataFlow/dataflow/dataflowagent/script/run_dfa_pipeline_prompt.py --task-description 我想写一个适用于金融问题的过滤器提示词 --op-name ReasoningQuestionFilter --file-path /mnt/DataFlow/lz/proj/agentgroup/ziyi/dataflow/DataFlow/test111
