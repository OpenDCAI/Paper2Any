...
# # ========== 全局图与状态 ==========
# #os.environ["DF_API_KEY"] = "sk-"

# graph = create_icongen_refine_loop_graph().build()
# global_state = MainState(request={"chat_api_url": "http://123.129.219.111:3000/v1"})
# last_image = None  # 仅保留最后一张，不需要历史

# # 确保 API Key
# os.environ.setdefault("DF_API_KEY", os.getenv("DF_API_KEY", "dummy"))

# def _merge_state(state, out):
#     """把 ainvoke 返回 dict 合并回 state"""
#     if isinstance(out, dict):
#         if not hasattr(state, "_vars") or state._vars is None:
#             state._vars = {}
#         if not hasattr(state, "agent_results") or state.agent_results is None:
#             state.agent_results = {}

#         state.request.update(out.get("request", {}) or {})
#         state._vars.update(out.get("_vars", {}) or {})
#         state.agent_results.update(out.get("agent_results", {}) or {})
#         return state

#     for k in ("request", "_vars", "agent_results"):
#         setattr(state, k, getattr(out, k, getattr(state, k, {})))
#     return state
...
# # -------- Gradio UI --------
# with gr.Blocks(title="IconGen Pro — Infinite Refine (No History)") as demo:
#     gr.Markdown("## 🎨 IconGen Pro — 无限图标迭代")

#     with gr.Row():
#         keywords = gr.Textbox(label="🎯 关键词首次生成")
#         style = gr.Textbox(label="✨ 风格")

#     gen_btn = gr.Button("🚀 生成初始图")
#     image_output = gr.Image(label="输出图像")

#     edit_prompt = gr.Textbox(label="✏️ 编辑提示词", placeholder="add neon gears")
