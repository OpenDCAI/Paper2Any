...
def _ensure_httpx_proxy_compat() -> None:
    """
    Some environments set SOCKS proxies (e.g. socks5h://...) but don't have
    the optional httpx SOCKS dependencies installed, which breaks `import gradio`.
    If SOCKS support is missing, drop SOCKS proxy envs to allow the app to start.
    """
    proxy_env_keys = [
        "ALL_PROXY",
        "all_proxy",
    ...
    values = {k: os.environ.get(k, "") for k in proxy_env_keys}
    if not any(v.startswith(("socks5h://", "socks5://", "socks4://")) for v in values.values()):
        return
    try:
        import socksio  # type: ignore  # noqa: F401
    except Exception:
        for k, v in values.items():
            if v.startswith(("socks5h://", "socks5://", "socks4://")):
                os.environ.pop(k, None)
        return

    # Some httpx versions don't recognize "socks5h://", normalize to "socks5://".
    for k, v in values.items():
        if v.startswith("socks5h://"):
            os.environ[k] = "socks5://" + v[len("socks5h://") :]
...
PAGE_SETS: dict[str, Optional[Set[str]]] = {
    ...
    # Hugging Face Space: only expose Paper2PPT + PPT Polish
    "space": {"paper2ppt", "ppt2polish"},
}
